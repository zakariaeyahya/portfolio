    ...
    >
      <motion.div
        className={`h-full w-full rounded-2xl relative overflow-hidden transition-all duration-500 border-0 ${
          isHovered === index ? "transform scale-105 shadow-2" : "shadow-lg hover:shadow-xlkk"
        }`}
        whileHover={{ y: -8 }}
      >
          ...
          <div className="relative mb-2 flex  items-center justify-between">
            ...
            <div className="flex items-center gap-1 mb-6">
              {Array.from({ length: testimonial.rating }).map((_, i) => (
                ...
                >
                  <Icon icon="solar:star-bold" className="text-yellow-500 text-2xl drop-shadow-sm" />
                </motion.div>
              ))}
          ...
          <div className="mt-auto  border-t border-gray-200/50 dark:border-gray-600/50">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="flex-1">
                ...
                <div className="text-gray-600 dark:text-gray-400 text-sm font-medium">{testimonial.role}</div>
                <div className="text-gray-500 dark:text-gray-500 text-sm">{testimonial.company}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="relative mb-2 flex items-center justify-between">
        ...
        <div className="text-center mb-6">
          <span className="inline-flex items-center gap-2 px-4 py-2 bg-sky-100 dark:bg-blue-900/30 text-sky-700 dark:text-blue-400 rounded-full text-sm font-medium">
            ...
            {portfolioContent.testimonials.featuredBadge}
          </span>
        </div>
      </div>
...
export default function TestimonialsSection() {
  ...
  const featuredTestimonial = testimonialItems.find((t) => t.featured);
  const otherTestimonials = testimonialItems.filter((t) => !t.featured);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <SectionHeader
          tagText={testimonials.sectionHeader.tagText}
          tagIcon={testimonials.sectionHeader.tagIcon}
          heading={testimonials.sectionHeader.heading}
          showUnderline={false}
          centered={true}
        />
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
              {testimonials.gridTitle}
            </h3>
            <p className="text-gray-600 dark:text-gray-400">{testimonials.gridDescription}</p>
          </div>
